...
To update screenshots (desktop, mobile, dark mode), follow these steps:

1. Install dependencies and Docker-specific dependencies if they haven’t been installed yet or were recently updated:
   ```bash
   yarn
   yarn test:pw:docker:deps
   ```
    It’s okay if the latter fails at the end.


2. Update screenshots:
   ```bash
   yarn test:pw:update <test-file>
   ```
   Example:
   ```bash
   yarn test:pw:update ui/snippets/searchBar/SearchBar.pw.tsx
   ```
