...
const SearchBarSuggest = ({ query, entitiesQuery, zetaChainCCTXQuery, externalSearchItem, searchTerm, onItemClick }: Props) => {
  ...
  const itemsGroups = React.useMemo(() => {
    if (!query.data && !zetaChainCCTXQuery.data?.items.length && !marketplaceApps.displayedApps && !entitiesQuery.data) {
      return {};
    }
    ...
    return map;
  }, [ query.data, marketplaceApps.displayedApps, searchTerm, zetaChainCCTXQuery.data?.items, entitiesQuery.data ]);
  ...
  const content = (() => {
    if (query.isPending || marketplaceApps.isPlaceholderData ||
        (config.features.zetachain.isEnabled && zetaChainCCTXQuery.isPending) || entitiesQuery.isPending) {
