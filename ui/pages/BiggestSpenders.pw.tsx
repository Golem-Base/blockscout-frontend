import React from 'react';

import type { ListBiggestSpendersResponse } from '@golembase/l3-indexer-types';

import BiggestSpenders from './BiggestSpenders';

const biggestSpenders: ListBiggestSpendersResponse = {
  items: [
    {
      rank: '1',
      address: '0xebC636b87B86218bE0657b928EEd42A1D573b20c',
      total_fees: '78345169982575750',
    },
    {
      rank: '2',
      address: '0xDDBF0d185f0225D59017f5EAbD3cFa902C75B03F',
      total_fees: '1914375000000000',
    },
    {
      rank: '3',
      address: '0xEB0680e8fEF19f5B6490a083d5b59f6F04930B5B',
      total_fees: '276051700',
    },
    {
      rank: '4',
      address: '0x32AAdf624ba6F7FCd76c72774E0A687be8f32dAC',
      total_fees: '10210000',
    },
    {
      rank: '5',
      address: '0x32C5fD6d7F6618926b5ec1ABe7440bF1d6aA98cB',
      total_fees: '10210000',
    },
  ],
  pagination: {
    page: '1',
    page_size: '5',
    total_pages: '2',
    total_items: '10',
  },
};

test('base view +@mobile +@dark-mode', async({ render, mockApiResponse }) => {
  await mockApiResponse('golemBaseIndexer:biggestSpenders', biggestSpenders);
  const component = await render(<BiggestSpenders/>);
  await expect(component).toHaveScreenshot();
});
